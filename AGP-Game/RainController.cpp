...
RainController::RainController(ID3D11Device* d3d11Device, ID3D11DeviceContext* d3d11DeviceContext)
{
	...
	rc_EyePosW = XMFLOAT3(0.0f, 0.0f, 0.0f);
	rc_EmitPosW = XMFLOAT3(0.0f, 0.0f, 0.0f);
...
void RainController::Draw(XMMATRIX* view, XMMATRIX* proj, camera* c, GameTimer* time)
{
	...
	cb_rain_values.EmitPos = rc_EmitPosW;
	cb_rain_values.EmitDirW = rc_EmitPosW;

	rc_pImmediateContext->UpdateSubresource(rc_cb0, 0, 0, &cb_rain_values, 0, 0);
	...
	std::swap(rc_DrawVB, rc_StreamOutVB); //hopefully the right method
...
ID3D11ShaderResourceView* RainController::CreateRandomTexture1DSRV()
{
	...
	{
		randomValues[i].x = RandF(-1.0, 1.0);
		randomValues[i].y = RandF(-1.0, 1.0);
		randomValues[i].z = RandF(-1.0, 1.0);
		randomValues[i].w = RandF(-1.0, 1.0);
	}
