...
camera* g_pCamera; //camera
ID3D11ShaderResourceView* g_pTexture0; //Texture pointer
ID3D11SamplerState* g_pSampler0; //sampler state for texture
//Lighting
XMVECTOR g_directional_light_shines_from;
XMVECTOR g_directional_light_colour;
XMVECTOR g_ambient_light_colour;
//Point light
XMVECTOR g_point_light_postition;
...
//reflective
Reflective* g_pReflective;


//rain
RainController* g_pRainController;

// Rename for each tutorial – This will appear in the title bar of the window
char		g_TutorialName[100] = "Tutorial 14 Exercise 01\0";
...
void CalculateFPS()
{
	...
	if((g_pGameTimer.TotalTime() - timeElapsed)>= 1.0f)
	{
		...
		outs.precision(6);
		outs << g_TutorialName << L" FPS: " << fps << L"Frame Time: " << mspf << L" (ms)";
		SetWindowText(g_hWnd, LPCSTR(outs.str().c_str()));
...
HRESULT InitialiseGraphics()
{
	...
	g_pSkybox->Initialize();
	
	g_pModel = new Model(g_pD3DDevice, g_pImmediateContext);
	g_pModel->LoadObjModel((char*)"Resources/Sphere.obj");

	g_pReflective = new Reflective(g_pD3DDevice, g_pImmediateContext);
	g_pReflective->LoadObjModel((char*)"Resources/cube.obj");

	
	
	g_pCamera = new camera(0.0f, 0.0f, -5.0f, 0.0f, 0);

	g_pRainController = new RainController(g_pD3DDevice, g_pImmediateContext);
	g_pRainController->Initialize();
...
void RenderFrame(void)
{
	...
	g_pReflective->SetZ(5.0f);
	g_pReflective->SetX(5.0f);
	g_pReflective->Draw(&view, &projection);
	
	g_pRainController->Draw(&view, &projection, g_pCamera, &g_pGameTimer);
